...
function updateComment(url, request) {
  ...
  } else if (requestComment && requestComment.body ) {

    const comment = {
      body: requestComment.body,
    }
    database.comments[savedComment.id].body = comment.body; // update comment
    response.status = 200;
    response.body = { comment: database.comments[savedComment.id].body };
  }
...
function deleteComment(url, request) {
  ...
  if (!savedComment) {
    ...
    let user = database.comments[id].username; // get the user where to delete the comment
    let articleId = database.comments[id].articleId; // get the article where to delete the comment

    database.comments[id] = null;
    //delete database.comments[id];
    // why are we leaving the id in the database with 'null'?
    // why not just delete the whole property with the id?

    // check the article, in which the comment was made & delete from array
    const commentArticleId = database.articles[articleId].commentIds;
    commentArticleId.splice(commentArticleId.indexOf(id), 1);

    // check the user who posted the comment & delete from array
    const commentUserId = database.users[user].commentIds;
    commentUserId.splice(commentUserId.indexOf(id), 1);
